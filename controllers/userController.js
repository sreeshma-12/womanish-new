...
async function getShopProducts(req, limit, count=false) {
    ...
    if (req.query.category) { // category
        products = productModel.find({ brand: new ObjectId(req.query.category) });
    } else if (req.query.search) { // search
...
const addaddress = async (req, res, next) => {
    try {
        ...
        await userModel.updateOne({ _id: id }, { $push: { useraddress: newaddresss } });
        res.redirect("/account");
    } catch (error) {
        next(error);
    }
...
const checkoutform = async (req, res, next) => {
    try {
        ...
        await Promise.all(
            user.cart.map((item) =>
                productModel.findByIdAndUpdate(item.product_id, {
                    $inc: {
                        stock: -item.quantity,
                    },
                })
